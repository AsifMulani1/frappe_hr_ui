    ...
    <div v-if="celebrations.length" class="flex flex-col gap-3">
      <div v-for="(c, i) in celebrations" :key="i" class="flex items-center gap-2.5">
        ...
        <div class="min-w-0 flex-1">
          <div class="text-sm font-medium text-ink-gray-9">{{ c.name }}</div>
          <div class="text-xs text-ink-gray-5">{{ c.label }} · {{ c.sub }}</div>
        </div>
        <Button variant="subtle" theme="gray" size="sm" label="Congrats" @click="congratulate(c)" />
      </div>
    </div>
    <div v-else class="py-4 text-center text-sm text-ink-gray-5">No celebrations this week.</div>
  </Card>
</template>
