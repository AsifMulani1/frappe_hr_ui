  ...
  <Card>
    ...
    <div v-if="people.length" class="flex flex-col gap-3">
      <div v-for="(p, i) in people" :key="i" class="flex items-center gap-2.5">
        ...
        <span class="whitespace-nowrap text-[11.5px] text-ink-gray-5">{{ p.until }}</span>
      </div>
    </div>
    <div v-else class="py-4 text-center text-[13px] text-ink-gray-5">Everyone's in today.</div>
  </Card>
</template>
