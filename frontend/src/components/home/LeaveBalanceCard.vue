...
<template>
  <Card>
    ...
    <div v-if="balances.length" class="grid grid-cols-4 gap-3.5">
      <div v-for="l in balances" :key="l.code" class="py-0.5">
        <div class="flex items-baseline gap-1.5">
          <span class="tnum text-4xl font-medium text-ink-gray-9">{{ l.balance }}</span>
          <span class="text-xs text-ink-gray-5">/ {{ l.total }}</span>
        </div>
        <div class="my-2 text-xs font-medium text-ink-gray-7">{{ l.type }}</div>
        <ProgressBar :value="l.used" :max="l.total || 1" :color="(LEAVE_THEME[l.color] || LEAVE_THEME.blue).bar" :height="5" />
        <div class="mt-1.5 text-2xs text-ink-gray-5">
    ...
    <div v-else class="py-6 text-center text-sm text-ink-gray-5">No leave allocated yet.</div>
  </Card>
</template>
