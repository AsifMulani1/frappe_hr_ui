...
<template>
  <div v-if="resource.loading && !resource.data" class="flex h-[55vh] items-center justify-center text-ink-gray-5">
    <div class="flex items-center gap-2 text-sm"><Icon name="dot" :size="18" class="animate-pulse" /> {{ loadingText }}</div>
  </div>
  ...
  <div v-else-if="resource.error" class="flex h-[55vh] flex-col items-center justify-center gap-2 text-center">
    ...
    <div class="text-md font-medium text-ink-gray-8">Couldn't load this page</div>
    <div class="max-w-md text-sm text-ink-gray-5">{{ resource.error.messages?.[0] || resource.error.message || "Something went wrong. Please try again." }}</div>
