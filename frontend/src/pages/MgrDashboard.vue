          ...
          <div class="grid border-t border-outline-gray-1" style="grid-template-columns: repeat(auto-fill, minmax(150px, 1fr))">
            <div v-for="t in d.team || []" :key="t.name" class="flex flex-col gap-2 border-b border-r border-outline-gray-1 p-3.5">
              <div class="flex items-center gap-2">
                ...
                <div class="min-w-0"><div class="truncate text-[12.5px] font-medium text-ink-gray-9">{{ t.employee_name.split(" ")[0] }}</div>
                  <div class="truncate text-[10.5px] text-ink-gray-5">{{ t.today.in !== "—" ? "In " + t.today.in : (t.today.note || "—") }}</div></div>
              </div>
        ...
        <Card>
          ...
          <div class="flex flex-col">
            <div v-for="(a, i) in d.approvals || []" :key="a.id" class="flex items-center gap-2.5 py-2.5" :class="i ? 'border-t border-outline-gray-1' : ''">
              ...
              <div class="min-w-0 flex-1"><div class="truncate text-[13px] font-medium text-ink-gray-9">{{ a.person }}</div>
                <div class="text-[11.5px] text-ink-gray-5">{{ a.detail }} · {{ a.amount }}</div></div>
              <StatusBadge :tone="a.tone" size="sm" :label="a.kind" />
            </div>
            <div v-if="!(d.approvals || []).length" class="py-3 text-center text-[13px] text-ink-gray-5">All caught up.</div>
          </div>
        ...
        <Card>
          ...
          <div v-if="(d.upcoming_leave || []).length" class="flex flex-col gap-3">
            <div v-for="(u, i) in d.upcoming_leave" :key="i" class="flex items-center gap-2.5">
              ...
              <div class="flex-1"><div class="text-[13px] font-medium text-ink-gray-9">{{ u.employee_name }}</div><div class="text-[11.5px] text-ink-gray-5">{{ u.leave_type }}</div></div>
              <span class="text-[11.5px] text-ink-gray-5">{{ u.when }}</span>
            </div>
          </div>
          <div v-else class="py-3 text-center text-[13px] text-ink-gray-5">No upcoming leave.</div>
        </Card>
      </div>
    </div>
  </div>
