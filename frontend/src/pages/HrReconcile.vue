...
<template>
  <div class="mx-auto max-w-[1100px] px-6 py-[22px]">
    <PageHeader title="Reconciliation" subtitle="Pre-disbursement validation">
      <template #actions><Button variant="solid" theme="gray" label="Mark reconciled"><template #prefix><Icon name="check" :size="15" /></template></Button></template>
    </PageHeader>
    ...
    <Card>
      ...
      <div class="flex flex-col">
        <div v-for="(c, i) in d.checks || []" :key="i" class="flex items-center gap-3 py-3.5" :class="i ? 'border-t border-outline-gray-1' : ''">
          ...
          <Button v-if="c.st !== 'pass'" variant="outline" theme="gray" size="sm" label="Resolve" />
        </div>
      </div>
    </Card>
  </div>
