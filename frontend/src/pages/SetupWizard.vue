  ...
  <div class="mx-auto max-w-[860px] px-6 py-[22px]">
      ...
      <template v-for="(label, i) in STEPS" :key="i">
        <Button variant="ghost" @click="step = i + 1">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs" :class="step === i + 1 ? 'font-medium text-ink-gray-9' : 'text-ink-gray-5'">{{ label }}</span>
          </div>
        </Button>
    ...
    <Card v-if="step === 1">
      ...
      <template v-if="status.data && !s.has_company">
        <div class="text-md font-medium text-ink-gray-9">Create Your Company</div>
        <p class="mt-1 text-sm text-ink-gray-6">Everything — employees, payroll, compliance — belongs to a company. Let's create yours to get started.</p>
        ...
        <p class="mt-3 text-xs text-ink-gray-4">This builds your chart of accounts — it can take a few seconds.</p>
      ...
      <template v-else>
        <div class="text-md font-medium text-ink-gray-9">Your Company</div>
        <p class="mt-1 text-sm text-ink-gray-6">Everything — people, payroll, compliance — belongs to this company. Confirm it and continue; the next five steps take you to your first payroll.</p>
        <div class="mt-4 flex items-center gap-3 rounded-lg border border-outline-gray-1 p-4">
          <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-surface-gray-2 text-md font-semibold text-ink-gray-7">{{ s.abbr || (s.company || "?").slice(0, 2).toUpperCase() }}</div>
          <div class="min-w-0 flex-1">
            <div class="truncate text-base font-medium text-ink-gray-9">{{ s.company || "—" }}</div>
            <div class="mt-0.5 text-xs text-ink-gray-5">{{ s.country || "India" }} · {{ s.currency || "INR" }}</div>
          </div>
          <div class="shrink-0 text-right"><div class="tnum text-3xl font-medium text-ink-gray-9">{{ s.employees ?? 0 }}</div><div class="text-2xs text-ink-gray-5">employees</div></div>
        </div>
    ...
    <Card v-else-if="step === 2">
      <div class="text-md font-medium text-ink-gray-9">India Payroll Defaults</div>
      <p class="mt-1 text-sm text-ink-gray-6">Standard salary components, leave types, a salary structure, a holiday list — and turns on the statutory engine (PT/ESI/LWF). Safe to re-run.</p>
      <div class="mt-4 grid grid-cols-2 gap-2">
        <div class="flex items-center justify-between rounded-md border border-outline-gray-1 px-3 py-2 text-sm"><span>Salary components</span><StatusBadge :tone="s.components ? 'success' : 'neutral'" size="sm" :label="`${s.components || 0}`" /></div>
        <div class="flex items-center justify-between rounded-md border border-outline-gray-1 px-3 py-2 text-sm"><span>Leave types</span><StatusBadge :tone="s.leave_types ? 'success' : 'neutral'" size="sm" :label="`${s.leave_types || 0}`" /></div>
        <div class="flex items-center justify-between rounded-md border border-outline-gray-1 px-3 py-2 text-sm"><span>Salary structure</span><StatusBadge :tone="s.has_structure ? 'success' : 'neutral'" size="sm" :label="s.has_structure ? 'Ready' : 'None'" /></div>
        <div class="flex items-center justify-between rounded-md border border-outline-gray-1 px-3 py-2 text-sm"><span>Statutory engine</span><StatusBadge :tone="s.statutory_enabled ? 'success' : 'neutral'" size="sm" :label="s.statutory_enabled ? 'On' : 'Off'" /></div>
      </div>
      ...
      <div class="mt-6 border-t border-outline-gray-1 pt-5">
        <div class="text-sm font-medium text-ink-gray-9">Statutory Registration Numbers</div>
        <p class="mt-0.5 text-xs text-ink-gray-5">Optional now, required to file — they print on registers and challans.</p>
    ...
    <Card v-else-if="step === 3">
      <div class="flex items-center justify-between">
        <div class="text-md font-medium text-ink-gray-9">Import Employees</div>
        <Button variant="ghost" size="sm" label="Download CSV Template" @click="downloadTemplate" />
      </div>
      <p class="mt-1 text-sm text-ink-gray-6">Columns: {{ cols.map((c) => c.label + (c.reqd ? '*' : '')).join(", ") }}. Departments & designations are created automatically.</p>
      <label class="mt-3 flex cursor-pointer items-center justify-center gap-2 rounded-md border border-dashed border-outline-gray-2 py-6 text-sm text-ink-gray-6 hover:bg-surface-gray-1">
      ...
      <div v-if="parsed.rows.length" class="mt-3">
        <div class="mb-1 text-xs text-ink-gray-5">{{ parsed.rows.length }} rows · preview</div>
      ...
      <div v-if="importRes.data" class="mt-3 rounded-md bg-surface-gray-1 p-3 text-xs">
    ...
    <Card v-else-if="step === 4">
      <div class="text-md font-medium text-ink-gray-9">Assign Compensation</div>
      <p class="mt-1 text-sm text-ink-gray-6">Set a salary structure, base and state for employees who don't have one. State drives Professional Tax & LWF.</p>
      <div class="mt-3 grid grid-cols-2 gap-3">
      ...
      <div v-if="!bulkRows.length" class="mt-3 rounded-md border border-outline-gray-1 py-7 text-center text-sm text-ink-gray-5">Everyone has a salary structure assigned. 🎉</div>
      <div v-else class="mt-3 overflow-hidden rounded-md border border-outline-gray-1">
        <div class="grid grid-cols-[1fr_120px_150px] gap-2 border-b border-outline-gray-1 bg-surface-gray-1 px-3 py-2 text-xs font-medium text-ink-gray-5"><span>Employee</span><span>Base (₹/mo)</span><span>State</span></div>
        <div class="max-h-[40vh] overflow-y-auto">
          <div v-for="row in bulkRows" :key="row.employee" class="grid grid-cols-[1fr_120px_150px] items-center gap-2 border-b border-outline-gray-1 px-3 py-2 last:border-b-0">
            <div class="min-w-0"><div class="truncate text-sm font-medium text-ink-gray-9">{{ row.employee_name }}</div><div class="truncate text-2xs text-ink-gray-5">{{ row.designation }}</div></div>
    ...
    <Card v-else-if="step === 5">
      <div class="text-md font-medium text-ink-gray-9">Preview Your First Payslip</div>
      <p class="mt-1 text-sm text-ink-gray-6">See a real payslip — PT, ESI, LWF, PF and TDS computed exactly as a live run would. <b>Nothing is created or saved here</b>; the actual run lives in Payroll, where you do it each month.</p>
      <div class="mt-3 max-w-xs"><DateField label="Payroll month" v-model="period" placeholder="Pick any date in the month" /></div>
      <div v-if="preview.data" class="mt-3 text-xs text-ink-gray-5">
      ...
      <div v-if="sample.loading" class="mt-3 text-xs text-ink-gray-5">Computing a sample payslip…</div>
      ...
      <div v-else-if="sample.data?.ok" class="mt-3 overflow-hidden rounded-lg border border-outline-gray-1">
        <div class="flex items-center justify-between gap-3 bg-surface-gray-1 px-4 py-3">
          <div class="min-w-0">
            <div class="truncate text-sm font-medium text-ink-gray-9">{{ sample.data.employee_name }}</div>
            <div class="truncate text-xs text-ink-gray-5">{{ sample.data.designation || "—" }} · {{ sample.data.state || "—" }} · {{ sample.data.period }}</div>
          </div>
        ...
        <div class="grid grid-cols-2 divide-x divide-outline-gray-1">
          <div class="p-4">
            <div class="mb-1.5 text-2xs font-medium uppercase tracking-wide text-ink-gray-5">Earnings</div>
            <div v-for="e in sample.data.earnings" :key="e.component" class="flex justify-between py-0.5 text-xs"><span class="text-ink-gray-7">{{ e.component }}</span><span class="tnum text-ink-gray-9">{{ inr(e.amount) }}</span></div>
            <div class="mt-2 flex justify-between border-t border-outline-gray-1 pt-2 text-xs font-medium text-ink-gray-9"><span>Gross</span><span class="tnum">{{ inr(sample.data.gross_pay) }}</span></div>
          </div>
          <div class="p-4">
            <div class="mb-1.5 text-2xs font-medium uppercase tracking-wide text-ink-gray-5">Deductions</div>
            <div v-for="dd in sample.data.deductions" :key="dd.component" class="flex justify-between py-0.5 text-xs"><span class="text-ink-gray-7">{{ dd.component }}</span><span class="tnum text-ink-gray-9">{{ inr(dd.amount) }}</span></div>
            <div class="mt-2 flex justify-between border-t border-outline-gray-1 pt-2 text-xs font-medium text-ink-gray-9"><span>Total</span><span class="tnum">{{ inr(sample.data.total_deduction) }}</span></div>
          </div>
        </div>
        <div class="flex items-center justify-between bg-green-50 px-4 py-2.5">
          <span class="text-xs font-medium text-green-800">Net pay</span>
          <span class="tnum text-md font-semibold text-green-800">{{ inr(sample.data.net_pay) }}</span>
        </div>
      </div>
      <div v-else-if="sample.data && !sample.data.ok" class="mt-3 rounded-md border border-outline-gray-1 bg-surface-gray-1 p-3 text-xs text-ink-gray-6">
    ...
    <Card v-else>
      <div class="flex flex-col items-center gap-2 py-6 text-center">
        ...
        <div class="text-lg font-medium text-ink-gray-9">You're Live 🎉</div>
        <p class="max-w-md text-sm text-ink-gray-6">Defaults applied, statutory engine on, people in, pay assigned and your first payroll run. Review compliance or head to the dashboard.</p>
