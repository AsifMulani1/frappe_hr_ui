      ...
      <div class="flex flex-col">
        <div v-for="([t, s, state, prog], i) in d.stages || []" :key="t" class="flex items-stretch gap-3.5">
          <div class="flex flex-col items-center">
            <div class="flex h-[26px] w-[26px] items-center justify-center rounded-full text-2xs font-medium"
          ...
          <div class="flex flex-1 items-start justify-between pb-4.5">
            <div><div class="text-base font-medium" :class="state === 'todo' ? 'text-ink-gray-5' : 'text-ink-gray-9'">{{ t }}</div>
              <div class="mt-px text-xs" :class="state === 'active' ? 'text-blue-600' : 'text-ink-gray-5'">{{ s }}</div></div>
            <span class="tnum text-xs font-medium text-ink-gray-7">{{ prog }}</span>
          </div>
        </div>
      </div>
    </Card>
