        ...
        <Card>
          ...
          <div v-if="(d.upcoming || []).length" class="flex flex-col">
            <div v-for="(iv, i) in d.upcoming" :key="i" class="flex items-center gap-3 border-t border-outline-gray-1 py-3 first:border-t-0">
              ...
              <div class="min-w-0 flex-1"><div class="truncate text-[13px] font-medium text-ink-gray-9">{{ iv.cand }}</div><div class="text-[11.5px] text-ink-gray-5">{{ iv.round }}</div></div>
              <span class="tnum text-[12px] text-ink-gray-6">{{ iv.when }}</span>
            </div>
          </div>
