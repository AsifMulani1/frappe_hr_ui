      ...
      <div class="flex flex-col">
        <div v-for="(e, i) in events" :key="i" class="flex gap-4 py-4" :class="i ? 'border-t border-outline-gray-1' : ''">
          <div class="w-14 shrink-0 rounded-md border py-1.5 text-center" :class="TONE_BG[e.tone]">
            <div class="tnum text-[20px] font-medium leading-none">{{ e.date }}</div>
            <div class="mt-0.5 text-[11px] uppercase tracking-[.03em]">{{ e.mon }}</div>
          </div>
          <div class="flex-1">
            <div class="mb-0.5 flex items-center gap-2"><span class="text-[14px] font-medium text-ink-gray-9">{{ e.title }}</span><StatusBadge :tone="e.tone === 'info' ? 'info' : e.tone" size="sm" :label="e.tag" /></div>
          </div>
