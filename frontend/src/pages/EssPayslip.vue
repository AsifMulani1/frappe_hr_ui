...
<template>
  <div class="mx-auto max-w-[1320px] px-6 py-[22px]">
    ...
    <div v-if="!slips.length" class="py-16 text-center text-sm text-ink-gray-5">No payslips yet.</div>

    <div v-else class="grid items-start gap-5" style="grid-template-columns: 260px minmax(0,1fr)">
      <Card :pad="false" class="p-2">
        <div class="px-2.5 py-2 text-xs font-medium text-ink-gray-5">All payslips</div>
        <div class="flex flex-col gap-0.5">
          <Button v-for="s in slips" :key="s.name" @click="select(s.name)"
            ...
            <div class="flex w-full items-center justify-between">
              <div>
                <div class="text-sm font-medium" :class="sel === s.name ? 'text-blue-700' : 'text-ink-gray-9'">{{ s.month }}</div>
                <div class="tnum text-xs text-ink-gray-5">{{ formatINR(s.net_pay) }} net</div>
              </div>
        ...
        <div v-if="det.name">
          <div class="flex flex-wrap items-start justify-between gap-3 border-b border-outline-gray-1 px-6 py-5">
            <div class="flex items-center gap-3">
              <div class="flex h-[38px] w-[38px] items-center justify-center rounded-md bg-blue-600 text-xl font-medium text-white">{{ (det.company || 'C').charAt(0) }}</div>
              <div>
                <div class="text-md font-medium text-ink-gray-9">{{ det.company }}</div>
                <div class="text-xs text-ink-gray-5">Payslip for {{ det.month }}</div>
              </div>
            </div>
          ...
          <div class="grid grid-cols-4 gap-4 border-b border-outline-gray-1 bg-surface-gray-1 px-6 py-4">
            <div v-for="[l, v] in [['Employee', det.employee_name], ['Employee ID', det.emp_number], ['Designation', det.designation], ['Paid days', `${det.payment_days} / ${det.total_working_days}`]]" :key="l">
              ...
              <div class="tnum mt-0.5 text-sm font-medium text-ink-gray-9">{{ v }}</div>
            </div>
          </div>

          <div class="grid px-6 pb-5 pt-2" style="grid-template-columns: 1fr 1px 1fr">
            <div class="pr-6">
              <div class="py-3.5 text-xs font-medium uppercase tracking-[.03em] text-ink-gray-5">Earnings</div>
            ...
            <div class="pl-6">
              <div class="py-3.5 text-xs font-medium uppercase tracking-[.03em] text-ink-gray-5">Deductions</div>
          ...
          <div class="mx-6 mb-6 flex flex-wrap items-center justify-between gap-2 rounded-[10px] border border-blue-100 bg-blue-50 px-5 py-4">
            <div>
              <div class="text-sm font-medium text-blue-700">Net pay for {{ det.month }}</div>
              <div class="text-xs text-ink-gray-6">Gross {{ formatINR(det.gross) }} − deductions {{ formatINR(det.total_deduction) }}</div>
            </div>
            <div class="tnum text-7xl font-medium tracking-tight text-blue-700">{{ formatINR(det.net) }}</div>
          </div>
        </div>
        <div v-else class="p-10 text-center text-sm text-ink-gray-5">Select a payslip.</div>
      </Card>
    </div>
    </AsyncShell>
  </div>
