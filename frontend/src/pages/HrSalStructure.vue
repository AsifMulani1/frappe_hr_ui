  ...
  <div class="mx-auto max-w-[1320px] px-6 py-[22px]">
    ...
    <Drawer :open="bulkOpen" title="Assign compensation" subtitle="Set structure, base & state for employees who don't have one yet" :width="680" @close="bulkOpen = false">
      <div class="flex flex-col gap-4">
        ...
        <div v-if="!bulkRows.length" class="rounded-md border border-outline-gray-1 py-8 text-center text-[13px] text-ink-gray-5">
          Everyone already has a salary structure assigned. 🎉
        </div>
        <div v-else class="overflow-hidden rounded-md border border-outline-gray-1">
          <div class="grid grid-cols-[1fr_120px_160px] gap-2 border-b border-outline-gray-1 bg-surface-gray-1 px-3 py-2 text-[11.5px] font-medium text-ink-gray-5">
          ...
          <div class="max-h-[44vh] overflow-y-auto">
            <div v-for="row in bulkRows" :key="row.employee" class="grid grid-cols-[1fr_120px_160px] items-center gap-2 border-b border-outline-gray-1 px-3 py-2 last:border-b-0">
              <div class="min-w-0">
                <div class="truncate text-[13px] font-medium text-ink-gray-9">{{ row.employee_name }}</div>
                <div class="truncate text-[11px] text-ink-gray-5">{{ row.designation }} · {{ row.department }}</div>
              </div>
        ...
        <p class="text-[11.5px] text-ink-gray-4">Only rows with a base salary are assigned. Employment state drives Professional Tax & LWF.</p>
      </div>
      <template #footer>
