#!/bin/bash
# CI install: bench + frappe v16 + erpnext + hrms + frappe_hr_ui on a fresh test site.
# Adapted from frappe/hrms's CI helper.
set -e
...
bench get-app frappe_hr_ui "${GITHUB_WORKSPACE}"
...
bench --verbose --site test_site install-app frappe_hr_ui
